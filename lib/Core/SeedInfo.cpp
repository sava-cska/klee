...
void SeedInfo::patchSeed(const ExecutionState &state, 
                         ref<Expr> condition,
                         TimingSolver *solver) {
  Constraints required(state.constraintInfos);
  ConstraintManager cm(required);
  cm.addConstraint(condition, state.prevPC);
    ...
    if (it2 != assignment.bindings.end()) {
      ...
      if (res) {
        ...
        cm.addConstraint(EqExpr::create(
            read, ConstantExpr::alloc(it2->second[i], Expr::Int8)), state.prevPC);
      } else {
        cm.addConstraint(isSeed, state.prevPC);
      }
    }
  }
  ...
  for (Assignment::bindings_ty::iterator it = assignment.bindings.begin(), 
    ...
    for (unsigned i=0; i<array->size; ++i) {
      ...
      if (res) {
        ...
        cm.addConstraint(EqExpr::create(
            read, ConstantExpr::alloc(it->second[i], Expr::Int8)), state.prevPC);
      } else {
        cm.addConstraint(isSeed, state.prevPC);
      }
    }
